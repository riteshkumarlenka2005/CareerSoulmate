        ...
        <div className="flex flex-col lg:flex-row gap-20">
          <div className="lg:w-1/3">
            ...
            <div className="p-6 rounded-2xl bg-blue-500/10 border border-blue-500/20">
              <span className="text-xs font-black text-blue-400 uppercase tracking-widest block mb-2">Platform Power</span>
          ...
          <div className="lg:w-2/3 space-y-12 relative">
            ...
            {pathways.map((path, i) => (
              <div key={i} className="relative pl-16 group">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 p-8 rounded-3xl bg-white/[0.04] border border-white/10 group-hover:bg-white/[0.04] transition-all">
                  <div>
                    <span className="text-xs font-black text-gray-300 uppercase tracking-widest block mb-2">Degree Path</span>
                  ...
                  <div className="md:border-x md:border-white/10 md:px-8">
                    <span className="text-xs font-black text-blue-500 uppercase tracking-widest block mb-2">Target Destiny</span>
                    <h4 className="text-white font-black uppercase tracking-wide">{path.career}</h4>
                  </div>
                  <div>
                    <span className="text-xs font-black text-cyan-500 uppercase tracking-widest block mb-2">Critical Skills ({path.nsqf})</span>
                    <div className="flex flex-wrap gap-2">
                      {path.skills.map((s, si) => (
                        <span key={si} className="px-2 py-1 rounded-md bg-white/5 text-xs font-bold text-gray-200 uppercase">{s}</span>
                      ))}
                    </div>
                  </div>
                </div>
