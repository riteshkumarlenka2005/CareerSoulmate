...
const PersonalityPage: React.FC<{ onNavigate: (page: any) => void }> = ({ onNavigate }) => {
      ...
      {view === 'intro' && (
        <section className="relative pt-28 pb-16 px-6 overflow-visible">
          ...
          <div className="max-w-4xl mx-auto relative z-10 text-center">
            <div className="inline-block px-4 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[10px] font-black tracking-widest uppercase mb-10">
            ...
            <div className="grid md:grid-cols-2 gap-8 text-left mb-16">
               <div className="p-8 rounded-[32px] bg-white/[0.02] border border-white/5">
                  ...
                  <p className="text-gray-400 text-sm leading-relaxed">Being introverted is as valuable as being extroverted. High structure is as useful as high flexibility. We look for alignment, not judgment.</p>
               </div>
               <div className="p-8 rounded-[32px] bg-white/[0.02] border border-white/5">
                  <h4 className="text-sm font-black uppercase text-cyan-500 mb-4 tracking-widest">Environment Sync</h4>
                  <p className="text-gray-400 text-sm leading-relaxed">By understanding your decision-making and social patterns, we can recommend roles where you'll naturally find 'flow' and avoid burnout.</p>
               </div>
            </div>

            <button 
              onClick={() => setView('assessment')}
              className="px-12 py-6 bg-indigo-600 hover:bg-indigo-500 text-white rounded-[24px] font-black transition-all shadow-2xl shadow-indigo-600/40 uppercase tracking-[0.2em] text-[10px]"
            >
        ...
        <section className="py-24 px-6 max-w-4xl mx-auto min-h-[70vh] flex flex-col animate-in zoom-in duration-500">
           <div className="flex justify-between items-center mb-16">
              <div className="space-y-2">
                 <p className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.4em]">Persona Lab</p>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {QUESTIONS[currentIdx].options.map((opt, i) => (
                      <button 
                        ...
                        className="group relative p-12 rounded-[56px] bg-white/[0.02] border border-white/5 hover:border-indigo-500 hover:bg-indigo-600/5 transition-all duration-500 flex flex-col items-center justify-center min-h-[220px]"
                      >
        ...
        <section className="py-24 px-6 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-1000">
           ...
           <div className="grid lg:grid-cols-2 gap-20 items-center mb-32">
                         ...
                         return (
                           <div key={trait.id} className="space-y-4">
                              <div className="flex justify-between items-end">
                                 ...
                                 <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{trait.description}</span>
                              </div>
                              <div className="relative h-2 w-full bg-white/5 rounded-full">
                                 <div className="absolute inset-0 flex justify-between items-center px-2">
                                    <span className="text-[8px] font-black text-gray-600 uppercase z-10">{trait.leftLabel}</span>
                                    <span className="text-[8px] font-black text-gray-600 uppercase z-10">{trait.rightLabel}</span>
                                 </div>
                                 <div 
              ...
              <div className="relative">
                 ...
                 <div className="relative p-12 rounded-[64px] border border-white/10 bg-[#080808] backdrop-blur-3xl shadow-2xl overflow-hidden">
                    ...
                    <h4 className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.4em] mb-10">AI Archetype Sync</h4>
                    <div className="space-y-10">
                       <div className="flex items-center gap-8">
                          <div className="w-24 h-24 rounded-[32px] bg-indigo-600/20 border border-indigo-500/30 flex items-center justify-center text-5xl">
                             {archetype.icon}
                          </div>
                          <div>
                             <h4 className="text-3xl font-black uppercase text-white tracking-tighter">{archetype.name}</h4>
                             <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1">Core Professional Persona</p>
                          </div>
                       </div>
                       ...
                       <div className="grid grid-cols-2 gap-6 pt-10 border-t border-white/5">
                          <div>
                             <p className="text-[8px] font-black text-gray-600 uppercase mb-2 tracking-widest">Ideal Workspace</p>
                             <p className="text-xs font-black text-white uppercase">{scores.social < 0 ? 'Private & Focused' : 'Active & Collaborative'}</p>
                          </div>
                          <div>
                             <p className="text-[8px] font-black text-gray-600 uppercase mb-2 tracking-widest">Collaboration Style</p>
           ...
           <div className="p-16 rounded-[64px] border border-white/10 bg-white/[0.01] text-center max-w-4xl mx-auto">
              ...
              <div className="flex flex-col sm:flex-row gap-6 justify-center">
                 <button onClick={() => onNavigate('ai-recs')} className="px-12 py-6 bg-indigo-600 hover:bg-indigo-500 text-white rounded-[24px] font-black transition-all shadow-2xl shadow-indigo-600/40 uppercase tracking-[0.2em] text-[10px]">Sync Recs with Persona</button>
