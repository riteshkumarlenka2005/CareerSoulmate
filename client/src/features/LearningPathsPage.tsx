    ...
    <div className="bg-[#050505] text-white min-h-screen pb-20 md:pb-32 lg:pb-40 animate-in fade-in duration-700 font-sans">
      ...
      <section className="relative pt-20 pb-10 px-4 md:pt-28 md:pb-16 md:px-6 border-b border-white/10">
        ...
        <div className="max-w-7xl mx-auto relative z-10 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3 sm:gap-4 mt-8 md:mt-12">
            {(['pathways', 'nep', 'nsqf'] as Tab[]).map(t => (
              <button key={t} onClick={() => setActiveTab(t)} className={`px-4 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-black uppercase tracking-wider sm:tracking-[0.15em] text-xs sm:text-sm transition-all border ${activeTab === t ? 'bg-blue-600 border-blue-400 text-white shadow-xl' : 'bg-white/5 border-white/10 text-gray-300 hover:text-white'}`}>
      ...
      <section className="py-8 sm:py-10 px-4 md:py-20 md:px-6 max-w-7xl mx-auto">
        {activeTab === 'pathways' && (
          <div className="grid lg:grid-cols-4 gap-8 lg:gap-12">
            <div className="lg:col-span-1 space-y-4">
              <h3 className="text-xs font-black text-gray-300 uppercase tracking-[0.4em] mb-4 sm:mb-6">Select Career Goal</h3>
              {PATHWAYS_DB.map(p => (
            ...
            <div className="lg:col-span-3">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-12 gap-6">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-black uppercase tracking-tight">Active <span className="text-blue-500">Journey.</span></h2>
                  <p className="text-gray-300 text-[10px] sm:text-xs font-black uppercase tracking-widest mt-2">{activePathway.title}</p>
                </div>
                <div className="w-full md:w-auto p-4 rounded-xl sm:rounded-2xl bg-white/[0.04] border border-white/10 flex flex-wrap sm:flex-nowrap justify-between sm:justify-start items-center gap-4 sm:gap-6">
                  <div><p className="text-[10px] sm:text-sm font-black text-gray-400 uppercase">Salary</p><p className="text-[10px] sm:text-xs font-black text-white">{activePathway.salaryRange}</p></div>
                  <div className="w-0 sm:w-px h-0 sm:h-8 bg-white/10" />
                  <div><p className="text-[10px] sm:text-sm font-black text-gray-400 uppercase">Growth</p><p className="text-[10px] sm:text-xs font-black text-blue-400">{activePathway.growth}</p></div>
                </div>
              </div>
              <div className="relative pl-6 sm:pl-16">
                <div className="absolute top-0 bottom-0 left-0 sm:left-4 w-px bg-gradient-to-b from-blue-500 via-cyan-500 to-purple-500 opacity-20" />
                <div className="space-y-8 sm:space-y-12">
                  {activePathway.steps.map((step, idx) => (
                    <div key={idx} className="relative flex flex-col items-start gap-4">
                      <div className="absolute -left-10 sm:-left-16 w-8 h-8 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl bg-black border-2 border-blue-500/50 flex flex-col items-center justify-center z-10 shadow-[0_0_20px_rgba(59,130,246,0.2)]">
                        <span className="text-[8px] sm:text-[10px] sm:text-sm font-black text-gray-300 uppercase">NSQF</span>
                        <span className="text-xs sm:text-xl font-black text-white leading-none">{step.nsqf}</span>
                      </div>
                      <div className="w-full p-4 sm:p-8 rounded-2xl sm:rounded-3xl bg-white/[0.04] border border-white/10 hover:border-blue-500/30 transition-all">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-4 sm:mb-6">
                          <span className="text-[10px] sm:text-xs font-black text-blue-500 uppercase tracking-widest px-2 py-1 bg-blue-500/10 rounded-lg inline-block w-fit">Phase 0{idx+1}</span>
                          <h3 className="text-lg sm:text-xl font-black uppercase text-white tracking-tight">{step.title}</h3>
                        </div>
                        <div className="flex flex-wrap gap-2 mb-4 sm:mb-6">
                          {step.outcomes.map(o => <span key={o} className="px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg sm:rounded-xl bg-white/5 text-[10px] sm:text-xs font-bold text-gray-300 uppercase">{o}</span>)}
                        </div>
                        <div className="flex flex-col sm:flex-row justify-between gap-4">
                          <div><p className="text-[10px] sm:text-[10px] font-black text-gray-400 uppercase mb-1">Duration</p><p className="text-sm font-black text-white">{step.duration}</p></div>
                          <div><p className="text-[10px] sm:text-[10px] font-black text-gray-400 uppercase mb-1">Credential</p><p className="text-sm font-black text-cyan-400">{step.certs[0]}</p></div>
                        </div>
                      </div>
                    </div>
                  ))}
