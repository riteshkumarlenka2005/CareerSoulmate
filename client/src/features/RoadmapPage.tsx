    ...
    <div className="bg-[#050505] text-white min-h-screen pb-40 animate-in fade-in duration-700 font-sans overflow-x-hidden">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-12">
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-8 pt-4">
                 <div>
                    <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest mb-1">Target Destiny</p>
                    <p className="text-lg font-black text-white uppercase">{STUDENT_PROFILE.targetGoal}</p>
                 </div>
                 <div className="w-px h-12 bg-white/10 hidden sm:block" />
                 <div>
                    <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest mb-1">Current Level</p>
                    <p className="text-lg font-black text-blue-400 uppercase">{STUDENT_PROFILE.currentLevel}</p>
                 </div>
                 <div className="w-px h-12 bg-white/10 hidden sm:block" />
                 <div>
                    <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest mb-1">Active Window</p>
                    <p className="text-lg font-black text-white uppercase">{STUDENT_PROFILE.timeline}</p>
                 </div>
              </div>
            </div>

            <div className="w-full lg:w-96 p-10 rounded-[48px] bg-white/[0.03] border border-white/10 backdrop-blur-xl relative overflow-hidden shadow-2xl">
               ...
               <p className="text-[10px] font-black text-blue-500 uppercase tracking-[0.4em] mb-4">Journey Sync</p>
               <div className="flex justify-between items-end mb-6">
                 <h3 className="text-5xl font-black text-white leading-none">{STUDENT_PROFILE.overallProgress}%</h3>
                 <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Alignment Rating: 9.8/10</span>
               </div>
               ...
               <button className="w-full py-4 bg-white text-black rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all">Download Roadmap Dossier</button>
            </div>
          </div>
        </div>
      </section>
              ...
              return (
                <div key={milestone.id} className={`relative flex flex-col md:flex-row gap-12 ${idx % 2 !== 0 ? 'md:flex-row-reverse' : ''}`}>
                  ...
                  <div className={`md:w-[45%] flex flex-col pt-4 ${idx % 2 !== 0 ? 'md:items-start text-left' : 'md:items-end text-right'}`}>
                     <span className="text-[10px] font-black text-blue-500 uppercase tracking-[0.4em] mb-2">{milestone.timeframe}</span>
                     ...
                     <p className={`text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-full inline-block ${
                  ...
                  <div className={`md:w-[45%] p-10 rounded-[56px] bg-white/[0.02] border border-white/5 shadow-2xl transition-all duration-500 hover:border-blue-500/40 relative overflow-hidden group ${isCurrent ? 'ring-2 ring-blue-500/20 bg-blue-600/5' : ''}`}>
                    {/* Animated Grid Overlay */}
                    <div className="absolute inset-0 opacity-[0.03] grid-pattern pointer-events-none" />

                    <div className="relative z-10 space-y-10">
                      <div className="space-y-4">
                        <h5 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-3">
                      ...
                      <div className="space-y-8">
                        {milestone.layers.map((layer, lIdx) => (
                          <div key={lIdx} className="space-y-3">
                            <p className="text-[9px] font-black text-blue-500 uppercase tracking-[0.3em]">{layer.category}</p>
                            <div className="flex flex-wrap gap-2">
                              {layer.items.map(item => (
                                <span key={item} className="px-3 py-1.5 rounded-xl bg-white/5 border border-white/5 text-[10px] font-bold text-gray-200 uppercase tracking-tight group-hover:bg-white/10 transition-all">
                                  {item}
                                </span>
                              ))}
                            </div>
                      ...
                      {milestone.planB && (
                        <div className="pt-8 border-t border-white/5">
                           <button 
                             ...
                             className="flex items-center gap-3 text-[10px] font-black text-orange-500 uppercase tracking-widest hover:text-orange-400 transition-colors"
                           >
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <button className="py-3 px-6 rounded-2xl bg-white/5 hover:bg-white/10 text-[9px] font-black uppercase tracking-widest border border-white/5 transition-all text-gray-400 hover:text-white">Resources</button>
                        <button 
                          onClick={() => toggleComplete(milestone.id)}
                          className={`py-3 px-6 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all ${
      ...
      <section className="py-24 px-6 bg-[#080808] border-y border-white/5">
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center gap-16">
          <div className="w-48 h-48 rounded-[64px] bg-blue-600/10 border border-blue-500/30 flex items-center justify-center shrink-0 shadow-2xl relative overflow-hidden">
          ...
          <div className="space-y-8">
            ...
            <div className="flex gap-4">
              <button onClick={() => onNavigate('explorer')} className="px-8 py-3 bg-white/5 border border-white/10 hover:bg-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Re-sync Assessment</button>
              <button onClick={() => window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' })} className="px-8 py-3 bg-blue-600 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-blue-600/20">Talk to Advisor AI</button>
            </div>
          </div>
        </div>
      </section>
