          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
             {topThree.map((rec, i) => (
               <div key={rec.id} className="relative group">
                  <div className="absolute -inset-0.5 bg-gradient-to-br from-blue-600 to-cyan-600 rounded-[32px] blur opacity-10 group-hover:opacity-30 transition duration-500"></div>
                  <div className="relative p-8 rounded-[32px] bg-black border border-white/10 hover:border-white/20 transition-all flex flex-col h-full">
                     ...
                     <p className="text-xs text-gray-500 font-bold uppercase leading-relaxed mt-auto">
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-16 gap-10">
          <div className="flex gap-2 bg-white/[0.03] p-1.5 rounded-2xl border border-white/5">
            <button 
              ...
              className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'pending' ? 'bg-blue-600 text-white shadow-xl' : 'text-gray-500 hover:text-white'}`}
            >
              Pending Recommendations
            </button>
            <button 
              onClick={() => setActiveTab('saved')}
              className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'saved' ? 'bg-blue-600 text-white shadow-xl' : 'text-gray-500 hover:text-white'}`}
            >
          ...
          <div className="flex flex-wrap justify-center gap-3">
             {CATEGORIES.map(cat => (
               <button 
                 ...
                 className={`px-6 py-2.5 rounded-full border text-[9px] font-black uppercase tracking-widest transition-all ${filterCategory === cat ? 'bg-blue-500/10 border-blue-500 text-blue-400' : 'bg-white/5 border-white/10 text-gray-500 hover:text-white'}`}
               >
                 {cat}
               </button>
             ))}
        ...
        <div className="grid grid-cols-1 gap-6">
          {filteredRecs.map(rec => (
            <div key={rec.id} className="group p-10 rounded-[48px] bg-white/[0.02] border border-white/5 hover:border-blue-500/30 transition-all duration-500 flex flex-col lg:flex-row items-center gap-12 relative overflow-hidden">
               {/* Impact Gauge Visual */}
               <div className="lg:w-24 lg:h-24 rounded-[32px] bg-blue-600/10 flex flex-col items-center justify-center shrink-0 border border-blue-500/20 group-hover:bg-blue-600 transition-all duration-500">
                  <span className="text-[10px] font-black text-blue-400 group-hover:text-blue-200 uppercase mb-1">ROI</span>
                  <span className="text-2xl font-black text-white">{rec.impactScore}</span>
               </div>

               <div className="flex-grow space-y-6">
                  <div className="flex flex-col md:flex-row md:items-center gap-4">
                     <span className="text-[10px] font-black text-blue-500 uppercase tracking-[0.3em]">{rec.category}</span>
                  ...
                  <div className="grid md:grid-cols-2 gap-10">
                     <div className="space-y-4">
                        <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest border-l-2 border-blue-600 pl-4">The Strategic Why</p>
                        <p className="text-sm text-gray-400 font-medium leading-relaxed italic">{rec.why}</p>
                     </div>
                     <div className="space-y-4">
                        <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest border-l-2 border-emerald-600 pl-4">Expected Dividend</p>
                        <p className="text-sm text-gray-400 font-medium leading-relaxed">{rec.benefit}</p>
                     </div>
                  </div>

                  <div className="flex flex-wrap gap-8 pt-4 border-t border-white/5">
                     <div className="flex flex-col">
                        <span className="text-[8px] font-black text-gray-600 uppercase mb-1 tracking-widest">Investment</span>
                        <span className="text-[11px] font-black text-white uppercase">{rec.timeRequired}</span>
                     </div>
                     <div className="flex flex-col">
                        <span className="text-[8px] font-black text-gray-600 uppercase mb-1 tracking-widest">Complexity</span>
                        <span className="text-[11px] font-black text-cyan-400 uppercase">{rec.difficulty}</span>
                     </div>
                  </div>
               </div>

               <div className="shrink-0 flex flex-col gap-3 min-w-[200px]">
                  <button className="w-full py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-xl shadow-blue-600/20 transition-all active:scale-95">Start Execution</button>
                  {activeTab === 'pending' ? (
                    <button onClick={() => handleAction(rec.id, 'saved')} className="w-full py-4 bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all">Save For Later</button>
                  ) : (
                    <button onClick={() => handleAction(rec.id, 'new')} className="w-full py-4 bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all">Move to Active</button>
                  )}
                  {/* E. FEEDBACK LOOP */}
                  <div className="flex justify-center gap-4 mt-2">
                     <button onClick={() => handleAction(rec.id, 'rejected')} title="Not Relevant" className="w-10 h-10 rounded-xl bg-white/5 hover:bg-red-500/20 flex items-center justify-center transition-all border border-white/5 group/reject">
                        <svg className="w-4 h-4 text-gray-500 group-hover/reject:text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                     </button>
                     <button onClick={() => alert('Feedback recorded. Neural models retraining...')} title="Highly Relevant" className="w-10 h-10 rounded-xl bg-white/5 hover:bg-emerald-500/20 flex items-center justify-center transition-all border border-white/5 group/approve">
                        <svg className="w-4 h-4 text-gray-500 group-hover/approve:text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 13l4 4L19 7" /></svg>
                     </button>
                  </div>
               </div>
            </div>
          ...
          {filteredRecs.length === 0 && (
            <div className="py-32 text-center bg-white/[0.01] border border-dashed border-white/10 rounded-[64px]">
               <p className="text-gray-500 font-black uppercase tracking-[0.3em]">No {activeTab} recommendations found in this category.</p>
            </div>
          )}
        </div>
      </section>
             ...
             <div className="space-y-10">
                ...
                <div className="flex items-center gap-6">
                   <div className="flex -space-x-4">
                      {[1,2,3,4].map(i => <div key={i} className="w-12 h-12 rounded-full border-4 border-[#080808] bg-blue-500/20 flex items-center justify-center text-[10px] font-black">AI</div>)}
                   </div>
                   <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Optimizing pathways for 12,402 learners right now.</p>
                </div>
             </div>
          </div>
        </div>
      ...
      <section className="py-40 px-6 text-center">
        <div className="max-w-4xl mx-auto">
           ...
           <div className="flex flex-col sm:flex-row gap-6 justify-center">
             <button onClick={() => onNavigate('roadmap')} className="px-12 py-6 bg-blue-600 hover:bg-blue-500 text-white rounded-[24px] font-black transition-all shadow-2xl shadow-blue-600/40 uppercase tracking-[0.2em] text-[10px]">
               Review My Roadmap
             </button>
